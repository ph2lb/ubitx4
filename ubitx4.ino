...
// enum for simple step menu allows selection of 10..10Khz steps by menu 
typedef enum {
  STEPMIN = 0,
  S10 = 0,
  S50 = 1,
...
// enum for simple band menu allows selection of 80..10m bands by menu 
typedef enum 
{
  BANDMIN = 0,
  B80M = 0,
...
void checkButtons(){
  ...
  if (getButton() == BUTTON_NONE )
    return;
  active_delay(50);
  if (getButton() == BUTTON_NONE) //debounce
    return;
...
void doTuning(){
  ...
  if (s){
    ...
    long freqUpperLimit = (long)getFrequencyUpperLimit();
    long freqLowerLimit = (long)getFrequencyLowerLimit();
   
    long freqStep = (long)getFrequencyStep();
    if (s > 0) {
      frequency += freqStep;
      if (frequency > freqUpperLimit)
        frequency = freqUpperLimit;
    }
    if (s < 0)
    {
      frequency -= freqStep;
      if (frequency < freqLowerLimit)
        frequency = freqLowerLimit;
    }
...
//  digitalWrite(FBUTTON, HIGH);
  ...
  pinMode(ANALOG_SPARE, INPUT_PULLUP);
